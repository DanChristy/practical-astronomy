...
def position_of_elliptical_comet(lct_hour, lct_min, lct_sec, is_daylight_saving, zone_correction_hours, local_date_day, local_date_month, local_date_year, comet_name):
	...
	time_since_epoch_years = (PM.cd_jd(greenwich_date_day,greenwich_date_month,greenwich_date_year)-PM.cd_jd(0,1,greenwich_date_year))/365.242191+greenwich_date_year-PCD.get_comet_data("Halley")['Epoch']
	mc_deg = 360*time_since_epoch_years/PCD.get_comet_data("Halley")['Period']
	mc_rad = math.radians(mc_deg-360*math.floor(mc_deg/360))
	eccentricity = PCD.get_comet_data("Halley")['Ecc']
	true_anomaly_deg = PM.degrees(PM.true_anomaly(mc_rad,eccentricity))
	lc_deg = true_anomaly_deg + PCD.get_comet_data("Halley")['Peri']
	r_au = PCD.get_comet_data("Halley")['Axis'] * (1-eccentricity*eccentricity)/(1+eccentricity*math.cos(math.radians(true_anomaly_deg)))
	lc_node_rad = math.radians(lc_deg - PCD.get_comet_data("Halley")['Node'])
	psi_rad = math.asin(math.sin(lc_node_rad)*math.sin(math.radians(PCD.get_comet_data("Halley")['Incl'])))

	y = math.sin(lc_node_rad)*math.cos(math.radians(PCD.get_comet_data("Halley")['Incl']))
	x = math.cos(lc_node_rad)

	ld_deg = PM.degrees(PM.atan2(x,y)) + PCD.get_comet_data("Halley")['Node']
