...
class test_angle_decimal_degrees(UT.TestCase):
	...
	def test_angle_to_decimal_degrees(self):
		...
		print("Angle to Decimal Degrees:  [Angle] {degrees}d {minutes}m {seconds}s = [Decimal Degrees] {decimalDegrees}".format(degrees=self.degrees,minutes=self.minutes,seconds=self.seconds,decimalDegrees=resultDecimalDegrees))
	...
	def test_decimal_degrees_to_angle(self):
		...
		print("Decimal Degrees to Angle:  [Decimal Degrees] {decimalDegrees} = [Angle] {degrees}d {minutes}m {seconds}s".format(decimalDegrees=resultDecimalDegrees,degrees=revertDegrees,minutes=revertMinutes,seconds=revertSeconds))
...
class test_ecliptic(UT.TestCase):
	...
	def test_mean_obliquity_of_the_ecliptic(self):
		...
		print("Mean obliquity of the ecliptic:  [Greenwich Date] {g_month}/{g_day}/{g_year} = [Obliquity] {obliquity}".format(g_month=g_month,g_day=g_day,g_year=g_year,obliquity=obliquity))
...
class test_nutation(UT.TestCase):
	...
	def test_nutation(self):
		...
		print("Nutation:  [GWDATE] {greenwich_month}/{greenwich_day}/{greenwich_year} = [NUTATION] [LON] {nut_in_long_deg} [OBL] {nut_in_obl_deg}".format(greenwich_month=self.greenwich_month,greenwich_day=self.greenwich_day,greenwich_year=self.greenwich_year,nut_in_long_deg=nut_in_long_deg,nut_in_obl_deg=nut_in_obl_deg))
